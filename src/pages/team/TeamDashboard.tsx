...
export function TeamDashboard() {
  ...
  // Team lead: teammates, books, assignments
  ...
  // Teammate PINs we've seen (from create or reset) - persisted so we can show them after refresh
  ...
  // Edit assignment progress (team lead or teammate)
  ...
  useEffect(() => {
    ...
    api
      .getBooks()
      .then((data) => {
        if (!cancelled) setTeamBooks(data);
      })
      .catch(() => {
        if (!cancelled) setTeamBooks([]);
      });
    return () => { cancelled = true; };
  }, [user?.role]);
  ...
  useEffect(() => {
    ...
    api
      .getAssignments()
      .then((data) => {
        if (!cancelled) setTeamAssignments(data);
      })
      .catch(() => {
        if (!cancelled) setTeamAssignments([]);
      });
    return () => { cancelled = true; };
  }, [user?.role]);
  ...
  const handleSaveProgress = (e: React.FormEvent) => {
    ...
    setEditProgressError(null);
    api
      .updateAssignment(editProgressAssignment.id, editProgressStatus, editProgressNotes || undefined)
      .then(() => {
        if (user?.role === 'team_lead') refreshTeamAssignments();
        else api.getMyBooks().then(setMyBooks);
        ...
        {loading ? (
          ...
          <>
            <Card className="mb-8">
              ...
              {myBooks.length === 0 ? (
                <p className="text-stone-600">You don&apos;t have any book assignments yet.</p>
              ) : (
                <ul className="space-y-3">
                  {myBooks.map((a) => (
                    <li
                      key={a.id}
                      className="flex flex-wrap items-center justify-between gap-2 py-3 border-b border-stone-100 last:border-0 last:pb-0"
                    >
            ...
            {user?.role === 'team_lead' && (
              <Card className="mb-8">
                <h3 className="text-lg font-semibold text-stone-900 mb-4">Team management</h3>
                <div className="grid gap-6 sm:grid-cols-2">
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-medium text-stone-900">Teammates</h4>
                      <button
                        type="button"
                    ...
                    {teammates.length === 0 ? (
                      <p className="text-sm text-stone-600">No teammates yet. Add one to assign books and track progress.</p>
                    ) : (
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="font-medium text-stone-900">Assign books</h4>
                      <button
                        type="button"
                        onClick={() => {
                          setAssignBookError(null);
                    ...
                    {teamBooks.length === 0 && (
                      <p className="text-sm text-stone-600">
                        Choose a book list first to add books to your team.{' '}
                        <Link to="/team/choose-list" className="text-primary-600 hover:text-primary-700 font-medium">
                          Choose book list
                        </Link>
                      </p>
                    )}
                ...
                {teamBooks.length > 0 && (
                  <div className="mt-6">
                    <h4 className="font-medium text-stone-900 mb-3">Books and assignments</h4>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true" aria-labelledby="edit-progress-title">
                <Card asModal className="max-w-md w-full p-6 shadow-lg">
                  <h2 id="edit-progress-title" className="text-lg font-semibold text-stone-900 mb-4">
                    ...
                    {user?.role === 'team_lead' && editProgressAssignment.user ? ` — ${editProgressAssignment.user.username}` : ''}
                  </h2>
                ...
                {team?.book_list_id != null && (
                  <Card hover className="flex flex-col">
                    ...
                    {challengeableTeammates.length === 0 ? (
                      <p className="mt-2 text-sm text-stone-500">No other teammates to challenge yet.</p>
                    ) : (
                      <ul className="mt-3 space-y-2 flex-1">
                        {challengeableTeammates.map((t) => (
